# FriDay Assistant AI - Full Developer Understanding Report
...
2. Create `FriDayUI` instance (`ui = FriDayUI("face.png")`).
3. Start a background thread that:
   - Waits for API key setup (`ui.wait_for_api_key()`)
   - Instantiates `FriDayLive(ui)`
   - Runs async event loop with `asyncio.run(FriDay.run())`
...
- In `FriDayLive.run()`, internet is checked via `OfflineVoiceBridge.has_internet()`.
...
- Games: `game_updater`
- Lifecycle/memory: `shutdown_FriDay`, `save_memory`

Special behavior:
- `save_memory` is intentionally silent in `_execute_tool` and does not produce user-facing speech.

## 8. UI Layer (`ui.py`)
`FriDayUI` is a fixed-size PySide6 desktop interface with:
...
3. Add dispatch branch in `FriDayLive._execute_tool`.
