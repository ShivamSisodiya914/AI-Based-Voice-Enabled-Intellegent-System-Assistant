# FriDay Frontend Deep Technical Understanding

## 1. Scope
This document explains the frontend/UI architecture, rendering pipeline, interactions, theming system, and runtime integration contract used by FriDay.
...
2. Shell painting
- FriDayUI.paintEvent() draws full-window background effects:
...
## 6. Animation Engine
Animation heartbeat:
- FriDayUI._anim_timer triggers _animate() every ~16 ms.
...
Main operator controls in FriDayUI:
- command input field
- send button
- mute/live button
- theme buttons
...
Backend expects FriDayUI to expose and support:
- muted boolean
...
5. Add selector button styling path in FriDayUI._style_theme_btn.
