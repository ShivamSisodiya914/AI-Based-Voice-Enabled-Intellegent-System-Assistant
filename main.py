...
class FriDayLive:
    ...
    def _build_offline_system_prompt(self) -> str:
        return (
            "You are FriDay running in offline fallback mode. "
    ...
    def _execute_offline_tool(self, tool_name: str, args: dict) -> str:
        ...
        try:
            ...
            elif tool_name == "shutdown_FriDay":
    ...
    def _speak_offline_reply(self, text: str):
        ...
        if not clean_reply:
            return
        self.ui.write_log(f"FriDay: {clean_reply}")
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        if name in sensitive_tools:
            ...
            if not allowed:
                print(f"[FriDay] 🔒 Unauthorized: {msg}")
        ...
        if not allowed:
            print(f"[FriDay] ⚠️ Rate limit: {message}")
            return types.FunctionResponse(
                id=fc.id, name=name,
    ...
    async def run(self):
        ...
        while True:
            ...
            self._live_reconnect_requested = False
            print(f"[FriDay] 🔄 Reconnecting in {reconnect_delay:.1f}s...")
...
def main():
    ...
    from ui import LoginScreen, FriDayUI
