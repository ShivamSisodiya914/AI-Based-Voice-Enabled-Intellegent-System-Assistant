...
class LoginScreen(QtWidgets.QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        self.setWindowTitle("FriDay — Secure Access")
        t = T()
        ...
        title_lbl = QtWidgets.QLabel("F r i D a y")
...
# MAIN UI
# ═══════════════════════════════════════════════════════════════════════════════
class FriDayUI(QtWidgets.QWidget):
    ...
    def __init__(self, face_path: str, size: tuple[int, int] | None = None) -> None:
        ...
        super().__init__()
        self.setWindowTitle("FriDay ASSISTANT V3")
        self.setWindowFlags(
    ...
    def _draw_status_row(self, p: QtGui.QPainter, t: dict, W: int, orb_cy: int, orb_r: int) -> None:
        ...
        if self.muted:
            ...
            stat = f"{'●' if self.status_blink else '○'} {self._FriDay_state}"
            sc_  = t["primary"]
    ...
    def _draw_footer(self, p: QtGui.QPainter, t: dict, W: int, H: int) -> None:
        ...
        p.drawText(
            ...
            "[F4] MUTE  ·  [F5] CYCLE  ·  [F11] FULLSCREEN  ·  Right-click orb for modes  ·  F.r.i.D.a.y v3.0"
        )
    ...
    def _orb_clicked(self) -> None:
        ...
        idx = states.index(self._FriDay_state) if self._FriDay_state in states else 0
    ...
    def _set_state_impl(self, state: str) -> None:
        self._prev_state  = self._FriDay_state
    ...
    def _show_setup_ui(self) -> None:
        ...
        dlg.setModal(True)
        dlg.setWindowTitle("FriDay — First Boot")
        dlg.setStyleSheet(f"""
        ...
        sub = QtWidgets.QLabel("Configure your FriDay neural core before awakening.")
...
if __name__ == "__main__":
    app = QtWidgets.QApplication(sys.argv)
    app.setApplicationName("FriDay v3")

    ui = FriDayUI(face_path="", size=(1040, 840))
    ui.show()

    def _demo() -> None:
        time.sleep(1.5)
        ui.state_requested.emit("LISTENING")
        ui.log_requested.emit("SYS: F.r.i.D.a.y v3 neural core awakened.")
        ...
        ui.log_requested.emit("SYS: F4=unmute · F5=theme · F11=fullscreen · Mouse=gaze tracking.")
        ui.log_requested.emit("SYS: Scroll wheel on orb to adjust animation intensity.")
        time.sleep(3.2)
        ui.log_requested.emit("FriDay: All neural pathways nominal. How may I assist?")
        time.sleep(2.8)
    ...
    def handle_cmd(text: str) -> None:
        ...
        ui.log_requested.emit(f"FriDay: Processing command — '{text}'")
        time.sleep(2.2)
        ui.state_requested.emit("LISTENING")
